...
displayHelp () {
	printf "\n" &&
	printf "${bold}${GRE}Script to copy Chromium source files over the Chromium source tree.${c0}\n" &&
...
mkdir -v -p ${CR_SRC_DIR}/out/Chromium/ &&

printf "\n" &&
printf "${YEL}Copying Chromium source files over the Chromium tree...${c0}\n" &&
# Copy libjxl src
cd ~/Chromium &&
printf "\n" &&
printf "${YEL}Copying Chromium-libjxl source for JPEG-XL Support...${c0}\n" &&
cp -r -v Chromium-libjxl/src/. ${CR_SRC_DIR}/ &&
...
cp -r -v Chromium_shell/. ${CR_SRC_DIR}/out/Chromium/ &&
cp -r -v pak_src/binaries/pak ${CR_SRC_DIR}/out/Chromium/ &&
cp -r -v pak_src/binaries/pak-win/. ${CR_SRC_DIR}/out/Chromium/ &&
...
patchThor () {
  ...
  printf "${YEL}libavcodec header patch for HEVC...${c0}\n" &&
	...
	cd ${CR_SRC_DIR} &&
	git apply --reject ./ftp-support-Chromium.patch &&
	...
	printf "\n" &&
	printf "${YEL}Patching for Chromium 2024 UI...${c0}\n" &&
	cd ${CR_SRC_DIR} &&
	git apply --reject ./Chromium-2024-ui.patch &&
	...
	printf "${YEL}Chromium WebUI patch...${c0}\n" &&
	git apply --reject ./Chromium_webui.patch &&
	printf "${YEL}Chromium Updater patch...${c0}\n" &&
	git apply --reject ./win_updater.patch &&
	printf "${YEL}Chromium Keyboard Shortcuts patch...${c0}\n" &&
...
printf "\n" &&
echo "Copying other files to \`out/Chromium\`" &&

# Add default_apps dir for Google Docs Offline extension.
mkdir -v -p ${CR_SRC_DIR}/out/Chromium/default_apps &&
cp -r -v infra/default_apps/. ${CR_SRC_DIR}/out/Chromium/default_apps/ &&

# Add initial preferences file to open Chromium welcome page on first run.
cp -v infra/initial_preferences ${CR_SRC_DIR}/out/Chromium/ &&
cp -v infra/thor_ver ${CR_SRC_DIR}/out/Chromium/ &&
...
copyMacOS () {
	...
	cp -v other/Mac/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
...
copyRaspi () {
	...
	cp -v arm/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
	cp -v other/thor_ver_linux/wrapper-raspi ${CR_SRC_DIR}/chrome/installer/linux/common/wrapper &&
	cp -v pak_src/binaries/pak_arm64 ${CR_SRC_DIR}/out/Chromium/pak &&
	#./infra/fix_libaom.sh &&
	printf "\n" &&
...
copyAVX512 () {
	...
	cp -v other/AVX512/thor_ver ${CR_SRC_DIR}/out/Chromium/ &&
	cp -v other/AVX512/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
...
copyAVX2 () {
	...
	cp -r -v other/AVX2/third_party/* ${CR_SRC_DIR}/third_party/ &&
	cp -v other/AVX2/thor_ver ${CR_SRC_DIR}/out/Chromium/ &&
	cp -v other/AVX2/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
...
copySSE4 () {
	...
	cp -v other/SSE4.1/thor_ver ${CR_SRC_DIR}/out/Chromium/ &&
	cp -v other/SSE4.1/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
...
copySSE3 () {
	...
	cp -v other/SSE3/thor_ver ${CR_SRC_DIR}/out/Chromium/ &&
	cp -v other/SSE3/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
...
copySSE2 () {
	...
	cp -v other/SSE2/thor_ver ${CR_SRC_DIR}/out/Chromium/ &&
	cp -v other/SSE2/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
...
copyCros () {
	...
	cp -v other/CrOS/Chromium_version.txt ${CR_SRC_DIR}/ui/webui/resources/text/ &&
	[ -f ${CR_SRC_DIR}/third_party/ffmpeg/ffmpeg_hevc_ac3.patch ] || patchAC3;
	printf "\n"
}
case $1 in
...
#printf "alias ${YEL}args${c0} = ${CYA}gn args out/Chromium${c0}\n" &&
#printf "alias ${YEL}gnls${c0} = ${CYA}gn ls out/Chromium${c0}\n" &&
...
cd ~/Chromium &&
cat ./logos/Chromium_ascii_art.txt &&
