...
def display_help():
    print("\nScript to make a portable Chromium .zip for Windows.\n")
...
def copy_installer():
    ...
    for installer_file in installer_files:
        src_path = os.path.normpath(
            os.path.join(cr_src_dir, "out", "Chromium", installer_file)
        )
...
def extract_and_copy_files(installer_name):
    ...
    shutil.copy("./Chromium.BAT", "./temp/")
    shutil.copy("./Chromium_SHELL.BAT", "./temp/")
...
def zip_files(installer_name):
    ...
    if "AVX2" in installer_name:
        zip_filename = f"Chromium_AVX2_{version}.zip"
    elif "AVX" in installer_name:
        zip_filename = f"Chromium_AVX_{version}.zip"
    elif "SSE3" in installer_name:
        zip_filename = f"Chromium_SSE3_{version}.zip"
    elif "SSE4" in installer_name:
        zip_filename = f"Chromium_SSE4_{version}.zip"
    else:
        zip_filename = f"Chromium_portable.zip"
...
def main():
    print(
        "\nNOTE: Place the Chromium .exe in this directory and ensure 7-Zip is in PATH.\n"
    )
