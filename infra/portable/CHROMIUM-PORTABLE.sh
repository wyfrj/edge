...
# after 30 days.
find "$(pwd)/.config/Chromium/Crash Reports/pending/" -mtime +30 \
...
export GNOME_DISABLE_CRASH_DIALOG=SET_BY_Chromium

# Set config home.
XDG_CONFIG_HOME=${XDG_CONFIG_HOME:-$(pwd)/.config}

usage () {
  echo "Chromium-browser [-h|--help] [--temp-profile] [options] [URL]"
  echo
...
exec -a "$0" "$HERE/Chromium" "--disable-machine-id" "--disable-encryption" "--user-data-dir=$PROFILE" "--disk-cache-dir=$CACHE" $CHROME_USER_FLAGS "$@"
