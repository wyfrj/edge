# Tips for debugging Chromium on Linux
...
*** promo
GDB-7.7 is required in order to debug Chromium on Linux.
***
...
### Basic browser process debugging
    ...
    gdb -tui -ex=r --args out/Chromium/Chromium --disable-seccomp-sandbox \
        http://google.com
...
includes Ubuntu and ChromiumOS/ChromiumOS), process A can attach to process B only if A is
an ancestor of B.
...
```
Chromium --no-sandbox --renderer-cmd-prefix='xterm -title renderer -e gdb --args'
```
...
debugger, send the "run" command to the debugger:
    ...
    Chromium --no-sandbox --renderer-cmd-prefix='xterm -title renderer -e gdb \
        -ex run --args'
...
```
gdb -x ~/debug/browser --args Chromium --no-sandbox --disable-hang-monitor \
...
Usually `ps aux | grep Chromium` will not give very helpful output. Try
`pstree -p | grep Chromium` to get something like

```
        |                      |-bash(21969)---Chromium(672)-+-Chromium(694)
        |                      |                           |-Chromium(695)---Chromium(696)-+-{Chromium}(697)
        |                      |                           |                           \-{Chromium}(709)
        |                      |                           |-{Chromium}(675)
        |                      |                           |-{Chromium}(678)
        |                      |                           |-{Chromium}(679)
...
Alternatively, you might find out the process ID from Chromium's built-in Task
...
just run gdb on the main process.
    ...
    gdb --args Chromium --single-process
...
You can debug multi-process Chromium using `rr -f [PID]`
...
`ulimit -c unlimited` should cause all Chromium processes (run from that shell) to
...
Run with `CHROME_IPC_LOGGING=1` eg.
    ...
    CHROME_IPC_LOGGING=1 out/Chromium/Chromium
...
We obey your system locale. Try something like:
    ...
    LANG=ja_JP.UTF-8 out/Chromium/Chromium
...
order listed. Alternatively, just do this:
    ...
    LANGUAGE=fr out/Chromium/Chromium
...
Without all the language data available, Chromium will use a mixture of your
system language and the language you run Chromium in.
...
If you break in a debugger during a drag, Chromium will have grabbed your mouse
...
*   run Chromium via [xtrace](http://xtrace.alioth.debian.org/)
