...
## Clones current LLVM being used by the Chromium Project, and builds a local LLVM toolchain with Polly to use the Polly optimizations in the main Chromium BUILD.gn
...
displayHelp () {
	...
	printf "${bold}${GRE}LLVM toolchain with Polly to use the Polly optimizations in the main Chromium BUILD.gn${c0}\n" &&
...
printf "${bold}${GRE}to use the Polly optimizations in the main Chromium BUILD.gn${c0}\n" &&
printf "${bold}${YEL}Use the --pgo flag to build LLVM with PGO.${c0}\n" &&
printf "${bold}${YEL}Use the --jobs or -j flag to set the number of jobs (should be less or equal to the number of CPU cores)${c0}\n" &&
printf "${bold}${YEL}Use the --verbose or -v flag to make Ninja show verbose commands.${c0}\n" &&
printf "${bold}${YEL}Use the --version flag to show version, and --help to show help.${c0}\n" &&
