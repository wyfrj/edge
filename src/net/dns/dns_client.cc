...
class DnsClientImpl : public DnsClient {
 const bool disable_chromium_dns_config =
     base::CommandLine::ForCurrentProcess()->HasSwitch("disable-chromium-dns-config");
 public:
 ...
 private:
  std::optional<DnsConfig> BuildEffectiveConfig() const {
    DnsConfig config;
    // In Chromium it is sufficient to have secure DoH enabled to give the overrides priority
    if (config_overrides_.dns_over_https_config && config_overrides_.secure_dns_mode && !disable_chromium_dns_config) {
