...
HomeButton::HomeButton(PressedCallback callback, PrefService* prefs)
  ...
  static const bool disable_chromium_icons =
      base::CommandLine::ForCurrentProcess()->HasSwitch("disable-chromium-icons");
  if (disable_chromium_icons) {
    SetVectorIcons(kNavigateHomeChromeRefreshIcon, kNavigateHomeTouchIcon);
  } else {
    SetVectorIcons(kNavigateHomeChromeRefreshChromiumIcon, kNavigateHomeTouchIcon);
  }
