...
def reorder_imports(input_dir, output_dir, architecture):
  ...
  input_image = os.path.join(input_dir, 'chromium.exe')
  output_image = os.path.join(output_dir, 'chromium.exe')
  ...
  for fname in glob.iglob(os.path.join(input_dir, 'chromium.exe.*')):
...
def main(argv):
  ...
  parser.add_option('-i', '--input', help='reorder chromium.exe in DIR',
      metavar='DIR')
  parser.add_option('-o', '--output', help='write new chromium.exe to DIR',
      metavar='DIR')
